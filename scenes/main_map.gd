...
const TokenScene = preload("res://scenes/Tokens.tscn")
	
func _unhandled_input(event):
	# Check if the event is a left mouse button click press
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.is_pressed():
		var new_token = TokenScene.instantiate()
		
		#Calculate the position in teh TokenLayer's local coordinates
		#get_global_mouse_position() gives world coordinates
		#to_local() converts world coordinates to coordinates relative to the TokenLayer Node
		...
		#Add the token to teh scene tree as a child of the TokenLayer
		token_layer.add_child(new_token)
		
		#consume the event so nothing else processes this click further down the tree
		get_viewport().set_input_as_handled()
		# --- Place Token Logic Here ---
		#pass # Placeholder, we'll add code next
